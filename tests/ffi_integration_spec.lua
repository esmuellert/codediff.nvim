...
describe("FFI Integration", function()
  ...
  it("Returns correct structure format", function()
    local result = diff.compute_diff({"a"}, {"b"})
    assert.are.equal("table", type(result), "Result should be a table")
    assert.are.equal("table", type(result.changes), "Should have changes array")
    assert.are.equal("table", type(result.moves), "Should have moves array")
    assert.are.equal("boolean", type(result.hit_timeout), "Should have hit_timeout flag")
  end)
  ...
  it("Changes have correct nested structure", function()
    ...
    assert.are.equal("table", type(mapping), "Mapping should be a table")
    ...
    assert.are.equal("table", type(mapping.inner_changes), "Should have inner_changes array")
  end)
  ...
  it("LineRange has correct fields", function()
    ...
    assert.are.equal("number", type(mapping.original.start_line), "start_line should be number")
    assert.are.equal("number", type(mapping.original.end_line), "end_line should be number")
  ...
  it("CharRange has correct fields", function()
    ...
    if #result.changes > 0 and #result.changes[1].inner_changes > 0 then
      ...
      assert.are.equal("table", type(inner.original), "Should have original CharRange")
      assert.are.equal("table", type(inner.modified), "Should have modified CharRange")
      assert.are.equal("number", type(inner.original.start_line))
      assert.are.equal("number", type(inner.original.start_col))
      assert.are.equal("number", type(inner.original.end_line))
      assert.are.equal("number", type(inner.original.end_col))
  ...
  it("Handles empty arrays", function()
    ...
    assert.are.equal("table", type(result.changes))
    assert.are.equal(0, #result.changes, "Should have no changes for identical empty inputs")
  end)
  ...
  it("Handles identical content", function()
    ...
    assert.are.equal(0, #result.changes, "Should have no changes for identical content")
  end)
  ...
  it("Can get version string", function()
    ...
    assert.are.equal("string", type(version), "Version should be a string")
