-- State-observed E2Es through the registered :CodeDiff command and completion.
-- These assert real buffers, panel lists and layouts. Embedded-UI specs cover
-- screen cells and naturally deferred display events separately.
...
describe("Command E2E (real dispatch + render)", function()
  ...
  it(":CodeDiff --staged <rev> — compares index against the given revision", function()
    ...
    assert.equals(":0", explorer.data.target_revision, "the comparison target is the index (:0)")
    assert.is_not_nil(explorer.data.base_revision, "the comparison base resolves to HEAD~1")
  end)
  ...
  it("visual line history works from an active diff buffer (#463)", function()
    ...
    assert.are.same({ 1, 2 }, history_session.panel.data.opts.line_range)
    assert.equals("file.txt", history_session.panel.data.opts.file_path)
  ...
  it(":CodeDiff -- <subdir> — auto-refresh keeps the pathspec scope (#74 regression)", function()
    ...
    -- The real refresh key must retain scope. Wait for the new in-scope entry
    -- before checking that the out-of-scope additions stayed hidden.
  ...
  it("completion offers subcommands and git refs", function()
    ...
    assert.is_true(vim.tbl_contains(complete("CodeDiff --r"), "--repo"), "offers --repo flag")
    assert.is_true(vim.tbl_contains(complete("CodeDiff pr "), "clean"), "offers PR cleanup")
    assert.is_true(vim.tbl_contains(complete("CodeDiff pr 12 --r"), "--remote"), "offers PR remote flag")
    assert.is_true(vim.tbl_contains(complete("CodeDiff pr 12 --b"), "--base"), "offers PR base flag")
    assert.is_true(vim.tbl_contains(complete("CodeDiff pr clean --a"), "--all"), "offers cleanup all flag")
    assert.is_false(vim.tbl_contains(complete("CodeDiff pr clean -a"), "-a"), "does not offer a short cleanup alias")
    assert.is_false(vim.tbl_contains(complete("CodeDiff pr 12 -- "), "HEAD"), "PR path scope excludes revisions")
  end)

  it("completion after -- offers file paths, not subcommands or refs (#74)", function()
    vim.cmd("lcd " .. repo.dir)
    repo.write_file("modules/net/n.txt", { "n" })
    local cands = vim.fn.getcompletion("CodeDiff -- modules/", "cmdline")
