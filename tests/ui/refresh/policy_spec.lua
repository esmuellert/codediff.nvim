describe("refresh dependency policy", function()
  ...
  it("does not refresh fixed revision or history lists for worktree-only events", function()
    ...
    assert.is_false(policy.panel_needed({ name = "explorer", view = { base_revision = string.rep("a", 40), target_revision = string.rep("b", 40) } }, { worktree = true }))
    assert.is_false(policy.panel_needed({ name = "explorer", view = { base_revision = "HEAD", target_revision = ":0" } }, { worktree = true }))
    assert.is_true(policy.panel_needed({ name = "explorer", view = {} }, { index = true }))
  end)
end)
